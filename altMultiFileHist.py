...
totalnans=0
def genHist(colname,col): #at present moment, generates a histogram for each column given, without saving it as a file, so we can stack them
...
def grab4columns(target):
    ...
    base_dir = "/mnt/c/Users/ciroj/Desktop/Parquet Files/TnP/MultifileHistograms/AllFilesData/"
    my_files = [
        base_dir + "AllRealData.parquet",
    ...
    data_file_A = hist_data["AllRealData.parquet"]
    data_file_B = hist_data["DY_postEE_2022.parquet"]
    global my_datasets
    my_datasets = {
    "Real Data": data_file_A,
    "Simulation Data": data_file_B
}
